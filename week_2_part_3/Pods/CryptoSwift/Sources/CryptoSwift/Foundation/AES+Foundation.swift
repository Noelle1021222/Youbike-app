...
extension AES {
    convenience public init(key:String, iv:String, blockMode:CipherBlockMode = .cbc, padding: Padding = PKCS7()) throws {
        guard let kkey = key.bridge().data(using: String.Encoding.utf8, allowLossyConversion: false)?.arrayOfBytes(), let iiv = iv.bridge().data(using: String.Encoding.utf8, allowLossyConversion: false)?.arrayOfBytes() else {
