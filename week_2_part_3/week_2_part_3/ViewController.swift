...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate {
    ...
    func MapBtnClicked(_ sender:UIButton)  {
...
//            self.navigationController?.pushViewController(segue.destinationViewController, animated: true)
    ...
    func fetch() {
        ...
        do {
            let results = try moc.fetch(fetchRequest) as! [Station]
    ...
    func removeData () {
        ...
        do {
            let stationItems = try moc.fetch(fetchRequest) as! [Station]
            for stationItem in stationItems {
                moc.delete(stationItem)
            }
            do {
                try moc.save()
            }
...
//
    func parseStation (){
        ...
        let token =  JWT.encode(.hs256("appworks")) { builder in
            ...
            builder.expiration = Date().addingTimeInterval(5*60)
        }
        var coreDataStation = stationObject()
        let moc = DataController().managedObjectContext

        Alamofire.request(.GET, URL(string: "http://52.34.47.148/v2/stations")!, encoding:.json , headers: ["Authorization" : "Bearer \(token)"]).responseJSON { response in
            ...
            if let JSON = response.result.value {
                ...
                if let dataArray = JSON["data"] as? [AnyObject] {
                    for thing in dataArray {
                        ...
                        let stationItem = NSEntityDescription.insertNewObject(forEntityName: "Station", into: moc) as! Station
    ...
    func loadSegment(_ offset:Int, size:Int) {
        ...
        func nextUpdate(){
            ...
            Alamofire.request(.GET, URL(string: "http://52.34.47.148/v2/stations?paging=\(self.stationNext)")!, encoding:.json , headers: ["Authorization" : "Bearer \(token)"]).responseJSON { response in
                if let JSON = response.result.value {
                    //get paging
                    //思路：觀察API文件,paging 字串用在哪裡 -> HTTP 後
                    //先得到 paging 字串
